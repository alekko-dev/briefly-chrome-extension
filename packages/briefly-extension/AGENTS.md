...
- Source lives in `src/`: background (`src/background`), content script (`src/content`), popup UI (`src/popup`), shared utilities (`src/utils`), and icons (`src/icons` / `public/icons`).
- The built Chrome extension is output to `dist/` via Vite and `@crxjs/vite-plugin`; load this folder in `chrome://extensions`.
- For new features, place DOM-facing logic in the content script, Chrome API / orchestration in the popup, and shared logic in `src/utils`.
...
- When adjusting OpenAI or YouTube integration, keep user-facing error messages clear and non-technical, following patterns in `src/utils/errors.ts` and `src/popup/App.tsx`.
